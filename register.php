...
<div class="login-form">
    ...
    ?>
		...
		<div class="form-group">
            <input type="text" class="form-control" name="user" placeholder="Username Untuk Akun Baru Anda" required="required">
        </div>
		<div class="form-group">
        ...
        <div class="form-group">
            <button type="submit" class="btn btn-primary btn-lg btn-block" name="tombol" value="Daftar">Sign in</button>
        </div>
		<div class="bottom-action clearfix">
            <label class="float-left form-check-label"><input type="checkbox"> Remember me</label>
        </div>
    </form>
    <p class="text-center small">Sudah Punya Akun?<a href="login.php">Masuk disini!</a></p>
	...
	if ($tombol) {
		if ($pass1 == $pass2) {
			...
			if ($hitung > 0 ) {
				...
				$input = $koneksi->query("INSERT INTO tb_login (kode, username, password, level, status, proses) VALUES('$kode', '$user', '$pass1', 'user',
				'offline', 'aktif')");
				$input .= $koneksi->query("INSERT INTO tb_user(kode, nama_user, pekerjaan, email, no_hp, foto) VALUES('$kode', '$nama_user', '', '$email', '', '') ");

				if ($input) {
					echo "<script>alert('Daftar Sudah Berhasil!'); location='login.php'</script>";
				} else {
